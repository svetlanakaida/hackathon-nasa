...
<html lang="en">
<head>
    <title>Code coverage report for src/accounts.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">src</a> accounts.js
    </h1>
    <div class='clearfix'>
...
/* Private */
&nbsp;
function newAccountTransaction(name, ownerPublicKey, activePublicKey, options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
  const option = {
    bytes: 8192,
    stakedNet: 1,
    stakedCpu: 1,
...
&nbsp;
  return this.eos.transaction(<span class="fstat-no" title="function not covered" >(t</span>r) =&gt; {
<span class="cstat-no" title="statement not covered" >    tr.newaccount({</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    tr.buyrambytes({</span>
      payer: this.config.orePayerAccountName,
      receiver: name,
      bytes: option.bytes,
    });
&nbsp;
<span class="cstat-no" title="statement not covered" >    tr.delegatebw({</span>
...
&nbsp;
function weightedKey(key, weight = <span class="branch-0 cbranch-no" title="branch not covered" >1)</span> {
  return {
    key,
    weight,
  };
...
&nbsp;
function weightedKeys(keys, weight = <span class="branch-0 cbranch-no" title="branch not covered" >1)</span> {
  return keys.map(key =&gt; weightedKey(key, weight));
}
&nbsp;
function newPermissionDetails(keys, threshold = <span class="branch-0 cbranch-no" title="branch not covered" >1,</span> weights = <span class="branch-0 cbranch-no" title="branch not covered" >1)</span> {
  return {
    accounts: [],
...
&nbsp;
function newPermission(keys, permName, parent = <span class="branch-0 cbranch-no" title="branch not covered" >'active',</span> threshold = <span class="branch-0 cbranch-no" title="branch not covered" >1,</span> weights = <span class="branch-0 cbranch-no" title="branch not covered" >1)</span> {
  return {
    parent,
    perm_name: permName,
...
async function addAuthVerifierPermission(oreAccountName, keys) {
  ...
  await this.eos.transaction(<span class="fstat-no" title="function not covered" >(t</span>r) =&gt; {
<span class="cstat-no" title="statement not covered" >    perms.forEach(<span class="fstat-no" title="function not covered" >(p</span>erm) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      tr.updateauth({</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    tr.linkauth({</span>
      account: oreAccountName,
      code: 'token.ore',
      type: 'approve',
...
&nbsp;
async function createOreAccountWithKeys(activePublicKey, ownerPublicKey, options = <span class="branch-0 cbranch-no" title="branch not covered" >{},</span> confirm = <span class="branch-0 cbranch-no" title="branch not covered" >false)</span> {
  const oreAccountName = options.oreAccountName || generateAccountName();
  let transaction;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (confirm) {
...
<span class="cstat-no" title="statement not covered" >    transaction = await newAccountTransaction.bind(this)(oreAccountName, ownerPublicKey, activePublicKey, options);</span>
  }
  return {
    oreAccountName,
    transaction,
...
&nbsp;
async function generateOreAccountAndKeys(ownerPublicKey, options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
...
&nbsp;
async function generateOreAccountAndEncryptedKeys(password, ownerPublicKey, options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
  const {
    keys,
    oreAccountName,
    transaction,
